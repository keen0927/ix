...
import { addListRequest, openToastPopupRequest } from '../modules/todos';
...
const ButtonWrite = styled(ButtonTypeCircle)<{ disabled: boolean }>`
  ...
  /* border: ${props =>
		props.disabled ? '1px solid red' : '1px solid blue'}; */
...
const TodoInsert: FC = () => {
	...
	const { currentListId, isAddingList } = useSelector(
	...
	const onSubmit = useCallback<(e: FormEvent) => void>(
		e => {
			...
			if (!todo || !todo.trim()) {
				const message = '게시글을 작성 해 주세요.';
				return dispatch(openToastPopupRequest(message));
			}
	...
	return (
		<form onSubmit={onSubmit}>
			<TodoInsertBlock>
				...
				<ButtonWrite type="submit" disabled={isAddingList}>
					<SvgWrite />
				</ButtonWrite>
			</TodoInsertBlock>
		</form>
